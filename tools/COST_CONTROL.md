...
| **外部影像生成 API** | OpenAI GPT Image 2、其他非 Bedrock 影像服務（按 token/張計費） | OpenAI 等（**動用前硬閘：見 §1.6**） |
...
6. **🚨 外部圖片 / 影像生成付費 API 硬閘（非 Bedrock，如 OpenAI GPT Image 2、其他第三方影像服務）**：
   - **動用前一律先預估成本**（每張單價 × 預估張數 → 換算「本次 / 每日 / 每月」），**經使用者確認後才可執行**。這是硬閘：**不預估、不確認，就不准打 API**——無論是自動化還是單次手動呼叫。
   - **預設仍優先走 Bedrock SD3.5**（算 AWS 帳、無 key）；要改用外部付費影像 API，需有明確理由（如指令遵從度、局部重繪 inpainting 品質）並在設計冊寫明。
   - 一旦接進 **bot / 對外服務**，比照 §1.3 **四件套**（帳本 + 月封頂 cap + 後台用量卡 + kill switch）控管。
   - 純**個人手動 CLI** 免四件套，但**至少要在該服務後台設「月度用量上限」**，且工具本身要在每次呼叫後印出成本估算。
