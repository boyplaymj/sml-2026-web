// 組 starter 素材審圖:上=7 張 sprite 一致性接觸表(共用調色盤驗收),下=實境站景面板。
// 用法: PW_PATH=... FONTCONFIG_FILE=... node compose_starter.js
const { chromium } = require(process.env.PW_PATH);
const fs = require('fs');
const D = '/tmp/train-starter';
...
.cell img{width:100%;image-rendering:pixelated;background:
  repeating-conic-gradient(#2a2b2e 0% 25%,#242528 0% 50%) 0/20px 20px;border-radius:6px}
...
.scene{position:relative;height:430px;overflow:hidden;image-rendering:pixelated;
  background:linear-gradient(#8fd0ff 0%,#bfe6f5 44%,#7bbf63 44%,#5c9e48 100%)}
.spr{position:absolute;image-rendering:pixelated;filter:drop-shadow(0 6px 4px #0004)}
.station{width:230px;left:70px;top:120px}
.loco{width:200px;left:560px;top:225px}
...
.btn.pri{background:#5865f2}.btn.acc{background:#3ba55d}
  ...
  <div class="h">① 8-bit 素材（64px 低解析度 · 共用 16 色調色盤 · 透明底棋盤格）<small>更粗像素、更少色、算圖更輕</small></div>
  <div class="sheet">${cells}</div>
  <div class="h">② 實境站景面板預覽<small>物件縮小,世界感更大 · Tier3 都市駅 + EF210</small></div>
  <div class="embed">
...
(async () => {
  ...
  await (await p.$('.wrap')).screenshot({ path: `${D}/starter_review.png` });
