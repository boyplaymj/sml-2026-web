...
# 參數模式:
#   單張:  compose_goshuin.py <vid> <YYYY-MM-DD> <outpath>   → 只產該款到 outpath(bot 蓋印時用,避免併發共用 out/ 互踩)
#   批次:  compose_goshuin.py [YYYY-MM-DD]                    → 產全 7 款到 goshuin_art/out/(日期可選,預設今天)
SINGLE = None
if len(sys.argv) == 4:
    SINGLE = (sys.argv[1], sys.argv[3]); DATE_STR = sys.argv[2]
else:
...
def compose(vid,big,crestch,sub,acc,bg_,out_override=None):
    ...
    out=out_override or f"goshuin_art/out/{vid}.png"; bg.save(out); print("ok",vid,out)
if SINGLE:
    _vid,_out=SINGLE
    _row=next((r for r in V if r[0]==_vid),None)
    if not _row: print("unknown version",_vid); sys.exit(2)
    compose(*_row,out_override=_out)
else:
