...
class ShrineFortuneDAO extends BaseDAO {
  ...
  // 開新 visit(覆蓋)。先 GetItem 拿舊 lastVisit(給進場 lazy 失礼判斷)→ 再 SET 新值。
  // 回傳舊的 lastVisit(可能 undefined=首次參拜/無紀錄)。
  async openVisit (discordId, now) {
    const key = { discordId: String(discordId) };
    const res = await this.ddb.send(new GetCommand({
      TableName: this.tableName,
      Key: key
    }));
    const oldLastVisit = res.Item ? res.Item.lastVisit : undefined;
    await this.ddb.send(new UpdateCommand({
      TableName: this.tableName,
      Key: key,
      UpdateExpression: 'SET lastVisit = :v',
      ExpressionAttributeValues: { ':v': { openAt: now, closed: false } }
    }));
    return oldLastVisit;
  }

  // 退場礼:標記本趟 visit 乾淨結束(closed=true)。
  async closeVisit (discordId) {
    await this.ddb.send(new UpdateCommand({
      TableName: this.tableName,
      Key: { discordId: String(discordId) },
      UpdateExpression: 'SET lastVisit.#c = :true',
      ExpressionAttributeNames: { '#c': 'closed' },
      ExpressionAttributeValues: { ':true': true }
    }));
    return true;
  }

  // 原子 append 一筆 buff 到 buffs 陣列(buffs 不存在時從空陣列起)。避免 read-modify-write 競態。
  async appendBuff (discordId, buff) {
    await this.ddb.send(new UpdateCommand({
      TableName: this.tableName,
      Key: { discordId: String(discordId) },
      UpdateExpression: 'SET buffs = list_append(if_not_exists(buffs, :empty), :b)',
      ExpressionAttributeValues: { ':empty': [], ':b': [buff] }
    }));
    return true;
  }
