...
def compose(s):
    ...
    for i,ln in enumerate(poem):
        cx=int(gx1-cw*(i+0.5)); vcol(d,cx,gy0+int(cell*0.1),ln,F(csz),ink,cell)
    # 下半六分項解說(右起左行)
    ...
    for i,ax in enumerate(AXES):
        ...
        vcol(d,cx,iy0+int(H*.012),ax,F(lsz),red if ji else (60,50,45),icell)
        vcol(d,cx,iy0+int(H*.012)+icell*2+6,s["items"][ax]["text"],F(isz),ink,icell)
    # 朱印
