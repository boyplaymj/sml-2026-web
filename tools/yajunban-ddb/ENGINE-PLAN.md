...
| 配點(天賦)✅ 一般天賦定稿 | 消耗點+授予節點(單一 conditional UpdateItem,D1) | ✅ `allotTalent` + config `growth.talents` + `GrowthEngine`(硬閘+軟閘+玻璃箱,[GROWTH-talent-DRAFT.md](./GROWTH-talent-DRAFT.md))。💎數值天賦 next | 已完成(產酸臂範例節點;真值待 section-talent) |
| 碎片兌換/技能/轉職 | 消耗+授予 | 🔶 `consumeItem` 有,**各自寫法未做** | 新 DAO 寫法數個 + engine |
