...
> **✅改走 Amazon Bedrock(commit 3844154)——付款死結已解、不需 API key/console 儲值**:原本 Anthropic console 儲值按鈕壞掉付不了(Max訂閱又不能付API)。改用 Bedrock:`@anthropic-ai/bedrock-sdk` `AnthropicBedrock({awsRegion:'ap-southeast-1'})`、model **`global.anthropic.claude-opus-4-8`**(Opus4.8需跨區 inference profile,`apac.`/直接on-demand都不行);甜甜 EC2 role `sml-claude-ec2` 已授 `bedrock:InvokeModel`(anthropic.* + inference-profile);**算 AWS 帳單、IAM 認證、無 key、無 SSM**。prompt caching 支援、usage 齊全→帳本照記、月度封頂沿用。`PUZZLE_AI_DISABLED=1` 可強制 stub。**實測端到端全過**:gate擋兇手不打LLM、案情提問阿凱入戲真回話、帳本記帳。
> (SSM 那套已棄用;`@aws-sdk/client-ssm` dep + role `sweetbot-ssm-read` policy 留著無害、可日後清。)
> **✅B 全數上線(2026-07-13,deploy c1b9f6d + 重啟)**:甜甜已重啟載新碼(`Logged in as SweetBot#5855!` 無 error)。電話 AI(Bedrock)、記帳看板、結束案件、作答費、關鍵字全生效。順帶帶上他人 Player.js(伯夷每日廢除)+ a9f0199(猜花時間20→30秒,提早於16:00)。**可直接封測**:`!解謎 shengyao-fall-coverup` → 後台推進到 S3 → ☎撥打匿名電話 → 阿凱回話 + 後台用量卡跳成本。
