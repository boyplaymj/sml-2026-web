...
- `chatCapture`（沿用）{ enabled, videoId, liveChatId, pageToken, lastPoll, lastCount, ended, endMissCount, liveChatVideoId } — **偵測器只 merge 寫 `{videoId, enabled}`；抓取器只 merge 寫 `{liveChatId, pageToken, lastPoll, lastCount, ended, endMissCount, liveChatVideoId}`**。`ended`=terminal 信號（capture 連續「解析不到 activeLiveChatId」達 `endMissCount`≥2 門檻寫 true；成功解析/poll/換片歸零），偵測器只讀不寫；`liveChatVideoId`=此 liveChatId 屬於哪支片，capture 換片重置與 legacy 收斂用。
