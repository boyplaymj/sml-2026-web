...
- **相鄰 epoch 平滑(可選 seed)**:`balance.weather.persistence` 讓前一 epoch 天氣有黏性(避免晴↔大雨硬跳),仍 deterministic。
  - 🔴 **不可從賽季起點一路遞迴算到今天**(Codex 提醒):否則預報前算 N 個 epoch 會退化成 O(距賽季起點),愈後期愈慢。用**有限窗口**(黏性只看前 `persistenceWindow` 個 epoch,如 1–2)或 **correlated hash**(`hash(districtId, epoch)` 與 `hash(districtId, epoch-1)` 做低頻關聯,O(1) 不遞迴)。任一 `weatherAt(district, epoch)` 呼叫都應為 **O(1)**、可任意跳算未來 epoch(預報)不需回放歷史。
...
- **fallback = 降級純模擬層(硬要求)**:下列**任一**出錯,一律**視為無真實警報、核心迴圈照跑模擬層**,絕不影響主結算/報錯(Codex 提醒:真實層每一環都可能失敗):
  - `world.realWeather` 缺失 / `fetchedAt` 過期(> `balance.weather.realStaleMinutes`,如 180)
  - **CWA parser** 出錯(改格式/欄位/回傳非預期)→ Lambda 該次不寫、保留上一筆或標記過期
  - **SSM key** 讀不到 / CWA 授權失敗 / API 逾時 → 同上
  - **DDB world row** 讀取失敗 → 遊戲端當作無警報
  - 讀取一律加 60–120s 記憶體快取,別每次結算打 DDB。(呼應 §17.1 工程要點)
...
```
- `weatherWindowId` = 該波天候的**穩定 id**,**同窗同 type 只擲一次 → 刷面板不重抽、不重複開事件**(同 survival deterministic 鐵律)。🔴 **依層別取不同 window id(Codex 提醒)**:
  - **模擬層**:`epoch@type`(每 epoch 一波,可)。
  - **真實層**:颱風/豪雨常**跨多個 epoch** → 必須用 `realWeather.startedAt@type`(整波同一 id),**否則同一波真實天候會每個 epoch 各擲一次事件**(一場颱風連開三次淹水)。
...
1. **模擬層純函式**:`weatherAt(districtId, epoch)` 零 DB、同輸入同輸出、**O(1)**(可任意跳算未來 epoch 做預報,不從賽季起點遞迴);各區獨立(同 epoch 不同區可不同天氣);型別**永不出 typhoon**;severity ≤ `simSeverityCap`。
...
6. **天候事件**:severity 過 gate 才擲;`hash(userId, weatherWindowId, type)` **同窗同型別只擲一次**(刷面板不重抽/不重複開);**window id 分層**——模擬層 `epoch@type`、真實層 `startedAt@type`(跨多 epoch 的一波颱風/豪雨只擲一次、不每 epoch 重開);fired 進 `activeEvents[]`,`durationDays` 2–3,惰性到期移除。
