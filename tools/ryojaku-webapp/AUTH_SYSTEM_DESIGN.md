...
> **狀態：✅ 設計定稿（5 決策全定案 2026-07-22），可交 Codex 實作。**
...
## 9. 決策（✅ 全數定案 2026-07-22）
1. **自建 vs Cognito** → ✅ **自建 Go 擴充**（userId 深度耦合、遷移期不抽換身分層；Cognito 留待日後 MFA/企業級再評估）。
2. **認證門檻** → ✅ **軟門檻**：未驗證可登入、可逛，**不可開團/入團**；後台可切硬。
3. **Google 撞 email 自動合併** → ✅ **接受，限 provider 已驗證 email**；絕不靠未驗證的 email 字串相同就合併。
4. **既有 13k 舊帳號** → ✅ **不群發、走 lazy 驗證**：不回頭動任何人，新註冊起生效；舊用戶下次做信任行為或改密碼時當場提示驗證一次（保護 SES 寄件信譽、不打擾休眠用戶）。
5. **登入法下限** → ✅ **強制每帳號至少一把可登入鑰匙**，解綁最後一把時擋（防孤兒帳號）。

> **狀態：設計定稿，可交 Codex 實作**（依 §8 端點/前端/基建清單；§4 兩表進 `infra/01-tables.yaml`）。
