...
const imgCands = (n,folder) => {
  ...
  if(folder==='matchup' || folder==='calendar')   // NEXT GAME / 月曆:找不到就借 profile 的圖
    return [`assets/players/${folder}/${n}.png`,`assets/players/profile/${n}.png`,`assets/players/full/${n}.png`,`assets/players/${n}.png`];
...
const FEATURED = {grp:'men', i:0};                 // 首頁「本週對戰」顯示哪一場
...
window.playSMLRadarAnimation = function(payload){
  ...
  for(let i=0;i<6;i++){
    const q = getPoint(MAX_RADIUS*(D[i]/100), radarAngles[i]); targetPositions.push(q); finalPts.push(`${q.x},${q.y}`);
    const cp = getPoint(MAX_RADIUS*(C[i]/100), radarAngles[i]); finalCareer.push(`${cp.x},${cp.y}`);
  }
